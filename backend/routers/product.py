...
def _validate_and_map_attributes(
    ...
    """
    cat_attrs = _get_category_attributes(category_id, db)
...
def _sync_attributes(
    ...
    # Thêm mới
    validated = _validate_and_map_attributes(product.category_id, attribute_inputs, db)
...
def create_product(
    ...
    if payload.attributes and payload.category_id:
        _validate_and_map_attributes(payload.category_id, payload.attributes, db)
    ...
    if payload.attributes and payload.category_id:
        cat_attrs = _get_category_attributes(payload.category_id, db)
        cat_attr_by_name = {a.name: a for a in cat_attrs}
...
def get_products(
    ...
    products = _apply_active_filter(db.query(Product)).offset(skip).limit(limit).all()
    result = [_build_product_response(p) for p in products]
...
def search_product(
    ...
    products = (
        _apply_active_filter(db.query(Product))
        .filter(
...
def get_product(
    ...
    product = _apply_active_filter(db.query(Product)).filter(Product.id == product_id).first()
    if not product:
        raise HTTPException(404, "Product not found")

    result = _build_product_response(product)
    print(f"Product response built: {result}")  # Debugging line
    try:
        redis_client.setex(cache_key, 30, json.dumps(result))
    except Exception as e:
        logger.error(f"Redis cache set error: {e}")
...
def update_product(
    ...
    if "attributes" in update_data and update_data["attributes"] is not None:
        ...
        if not update_data["attributes"]:
            ...
            _sync_attributes(product, update_data["attributes"], db)
        else:
            raise HTTPException(
                status_code=400,
                detail="Cannot set attributes without a category",
